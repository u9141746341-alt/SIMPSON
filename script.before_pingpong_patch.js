...
function startRunner(){
  ...
  const loop = () => {
    ...
    // Always move calmly to the right
    x += Math.max(vx, window.innerWidth * 0.0038);

    // Clamp to right edge and stop there (no overshoot, no bounce)
    if (x >= maxX){
      x = maxX;
      runner.style.left = x + 'px';
      runner.style.transform = 'scaleX(1)'; // face right, no vertical hop
    ...
    runner.style.left = x + 'px';
    // No hop, no bounce; just face right
