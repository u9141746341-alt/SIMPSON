...
setTimeout(()=>{
  // in case the user doesn't click, show the button after a delay
...
function startRunner(){
  // Keep current X if set; otherwise start near the left edge
  let currentLeft = parseFloat(getComputedStyle(runner).left);
  let x = isNaN(currentLeft) ? 20 : currentLeft;

  const loop = () => {
    const rect = runner.getBoundingClientRect();
    const maxX = Math.max(10, window.innerWidth - rect.width - 20);
    const spd = Math.max(vx, window.innerWidth * 0.0038);

    // Move in current direction
    x += dir * spd;

    // Clamp + reflect at edges (no overshoot, no leaving the screen)
    if (x <= 10){
      x = 10;
      dir = 1; // go right
    }
    ...
    // Apply position and face the direction of travel
    runner.style.left = x + 'px';
    runner.style.transform = `scaleX(${dir})`; // no vertical hop
